...
const ROUTE = "/admin/catalogo/categorias/bicicletas";
...
test.describe("Catálogo · Categorías de bicicletas (árbol)", () => {
  test("crea una raíz y una subcategoría, edita y elimina ambas", async ({ page }) => {
    const rootName = uniqueName("E2E Categoría Raíz");
    const childName = uniqueName("E2E Subcategoría");
    const editedChildName = uniqueName("E2E Subcategoría Editada");
  ...
  test("cancelar en el modal de creación no crea ninguna categoría", async ({ page }) => {
    const name = uniqueName("E2E Categoría Cancelada");
