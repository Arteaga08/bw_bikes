...
/**
 * Seeds 20 real end-to-end orders (Stripe test mode, real webhooks, real
 * Resend/Telegram notifications) spread over the last two weeks, covering
 * every reachable `OrderStatus`. Two purposes at once:
 *
...
async function waitForStatus(orderId: string, targets: readonly string[]): Promise<string> {
  ...
  while (Date.now() < deadline) {
    ...
    if (!current) break;
    last = current.status;
    if ((targets as readonly string[]).includes(last)) return last;
    await new Promise((resolve) => setTimeout(resolve, WEBHOOK_POLL_INTERVAL_MS));
  }
  fail(
    `Timed out waiting for order ${orderId} to reach [${targets.join(", ")}] (still "${last}"). ` +
      "Is `stripe listen` running, and is STRIPE_WEBHOOK_SECRET current?",
  );
}

async function confirmPayment(stripe: Stripe, orderId: string): Promise<string> {
...
/**
 * 20 entries, 9 reachable statuses (`authorized` is transient by design —
 * see the module header). Split roughly half `in_stock` / half `on_request`
 * so both capture paths — and both notification shapes — get exercised.
 */
...
async function run(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < plan.length; i++) {
      ...
      let status = await confirmPayment(stripe, order.id);
    ...
    const dates = buildTwoWeekDateSpread(results.length);
    ...
    logger.info("");
    logger.info("[seed-batch-orders] Listo. 20 órdenes en /admin/ordenes:");
    for (const r of results) {
      logger.info(`  ${r.orderNumber} — objetivo: ${r.targetStatus} — final real: ${r.finalStatus}`);
    }
    logger.info("");
