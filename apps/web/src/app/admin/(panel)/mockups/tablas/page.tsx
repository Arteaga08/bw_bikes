...
import { DataTable, type DataTableColumn, TableRowActions } from "@/components/ui/DataTable";
...
const RULE_COLUMNS: DataTableColumn<MockSpecTemplateRow>[] = [
  ...
  { key: "fields", header: "Campos", align: "right", render: (row) => String(row.fieldCount) },
  {
    key: "source",
    header: "Origen",
    align: "left",
    render: (row) => <Badge variant="neutral">{row.source === "manual" ? "Manual" : "Automática"}</Badge>,
  },
  {
    key: "status",
    header: "Estatus",
...
const SEPARATOR_COLUMNS: SeparatorDemoColumn[] = [
  ...
  { key: "fields", header: "Campos", kind: "number", render: (row) => String(row.fieldCount) },
  {
    key: "source",
    header: "Origen",
    kind: "status",
...
const ALIGN_CLASS: Record<"left" | "right", string> = { left: "text-left", right: "text-right" };
 ...
 */
function SeparatorDemoTable({ treatment }: { treatment: "loose" | "header-rule" | "full-rule" }) {
  ...
  return (
    <div className="overflow-x-auto rounded-card border border-borde bg-surface">
      <table className="w-full min-w-[36rem] table-auto border-collapse text-left">
        <thead>
              ...
              return (
                <th
                  ...
                  className={`${thPad} py-sm font-ui text-caption uppercase tracking-[1px] text-grafito ${ALIGN_CLASS[KIND_ALIGN[column.kind]]} ${
                    treatment === "header-rule" && !isLast ? "border-r border-borde" : ""
                  }`}
                >
                  {column.header}
                </th>
        ...
        <tbody>
          {MOCK_SPEC_TEMPLATES.map((row) => (
            <tr key={row.id} className="border-b border-borde last:border-b-0 hover:bg-base">
              {SEPARATOR_COLUMNS.map((column, index) => {
                const isLast = index === SEPARATOR_COLUMNS.length - 1;
                return (
                  <td
                    ...
                    className={`${tdPad} py-sm font-body text-body text-negro ${ALIGN_CLASS[KIND_ALIGN[column.kind]]} ${
                      treatment === "full-rule" && !isLast ? "border-r border-borde" : ""
                    }`}
                  >
                    {column.render(row)}
                  </td>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-lg p-md sm:p-lg">
        ...
        >
          <DataTable columns={RULE_COLUMNS} rows={MOCK_SPEC_TEMPLATES} getRowKey={(row) => row.id} />
        </Section>
        ...
        >
          <div className="flex flex-col gap-lg">
            <div className="flex flex-col gap-xs">
              <p className="font-ui text-ui text-negro">A · Solo más aire (px-lg, sin filete)</p>
              <SeparatorDemoTable treatment="loose" />
            </div>
            <div className="flex flex-col gap-xs">
              <p className="font-ui text-ui text-negro">B · Filete solo en el encabezado (recomendado)</p>
              <SeparatorDemoTable treatment="header-rule" />
            </div>
            <div className="flex flex-col gap-xs">
              <p className="font-ui text-ui text-negro">C · Filete de altura completa</p>
